...
class _RegistrationPageState extends State<RegistrationPage> {
  ...
  bool _hasPopulatedSelfInfo = false;
    ...
    return BlocProvider.value(
      ...
      child: BlocListener<RegistrationBloc, RegistrationState>(
        listener: (context, state) {
          final info = state.selfInformation?.data;
          if (info == null) return;
          if (_hasPopulatedSelfInfo) return;
          if (!mounted) return;
          // Populate fields from self information
          setState(() {
            _nameController.text = info.firstName ?? '';
            _surnameController.text = info.lastName ?? '';
            _emailController.text = info.email ?? '';
            _dobController.text = info.birthdate ?? '';
            _phoneController.text = (info.phone ?? '').toString();
            _emergencyPhoneController.text = (info.emergencyPhone ?? '')
                .toString();
            _cityController.text = (info.city ?? '').toString();
            _teamController.text = (info.teamName ?? '').toString();

            // validations
            _isNameValid = _nameController.text.trim().isNotEmpty;
            _isSurnameValid = _surnameController.text.trim().isNotEmpty;
            _isDobValid = _dobController.text.trim().isNotEmpty;
            _isEmailValid = _isEmailFormatValid(_emailController.text.trim());
            _isPhoneValid = _phoneController.text.trim().length == 10;
            _isEmergencyPhoneValid =
                _emergencyPhoneController.text.trim().length == 10;
            _isCityValid = _cityController.text.trim().isNotEmpty;

            // match gender and state from loaded lists
            if (state.genders.isNotEmpty && info.genderId != null) {
              final matches = state.genders
                  .where((g) => g.id == info.genderId)
                  .toList();
              _selectedSex = matches.isNotEmpty ? matches.first : null;
              _isSexValid = _selectedSex != null;
            }
            if (state.states.isNotEmpty && info.stateId != null) {
              final matches = state.states
                  .where((s) => s.id == info.stateId)
                  .toList();
              _selectedState = matches.isNotEmpty ? matches.first : null;
              _isStateValid = _selectedState != null;
            }
            // reset category/jersey selections when self info changes
            _selectedCategory = null;
            _selectedJersey = null;
          });

          // mark handled so subsequent state updates don't re-process same self info
          _hasPopulatedSelfInfo = true;

          // If we have gender and birthdate, request categories and tshirt sizes
          if (info.genderId != null) {
            _bloc.add(
          ...
          if (info.genderId != null && (info.birthdate?.isNotEmpty ?? false)) {
            setState(() {
                                        ...
                                        onChanged: (String? val) {
                                          setState(() {
                                            ...
                                            if (_isSelfRegistration) {
                                              // Al registrarse uno mismo, limpiar selección de corredores
                                              _selectedRunner = null;
                                              _selectedRunnerId = 0;
                                              // Reset marker so listener will populate exactly once
                                              _hasPopulatedSelfInfo = false;
                                              // Solicitar información del propio usuario
                                              _bloc.add(
                                                const FetchSelfInformationRequested(),
                                              );
                                            } else {
                                              // Al elegir "Otra persona" asegurarse de tener la lista de corredores
                                              _hasPopulatedSelfInfo = false;
                                              _bloc.add(
